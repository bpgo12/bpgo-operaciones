(function () {
  ...
  function render(panel, data) {
    ...
    var checks = (data.checks || []).map(function (check) {
      return '<li class="' + (check.configured ? "ready" : "pending") + '"><span>' + (check.configured ? "✓" : "○") + '</span><div><strong>' + escapeHtml(check.label) + '</strong><small>' + (check.configured ? "Configurado" : "Pendiente después de la aprobación") + '</small></div></li>';
    }).join("");
    panel.innerHTML = '<header><div><p class="eyebrow">Integración propia BPGO · Meta</p><h3>Coexistencia y registro integrado</h3><p>El mismo número podrá continuar en WhatsApp Business y conectarse con operaciones.bpgo.cl cuando Meta habilite la configuración.</p></div><span class="onboarding-badge ' + (data.connected ? "connected" : "review") + '">' + (data.connected ? "Conectado" : "Verificación en revisión") + '</span></header>' +
      '<ul class="onboarding-checks">' + checks + '</ul>' +
      '<div class="onboarding-action"><div><strong>' + (data.connected ? "Integración oficial activa" : data.readyToStart ? "Todo preparado para vincular el número" : "Esperando aprobación y Config ID de Meta") + '</strong><p>' + (data.connected ? "La credencial se encuentra cifrada y la plataforma puede recibir y responder mensajes." : "No se modificará el número ni la aplicación móvil hasta iniciar el flujo oficial de coexistencia.") + '</p></div>' +
  ...
  async function finishSignup(panel) {
    ...
    showStatus(panel, "Validando la cuenta y cifrando las credenciales…", "pending");
    try {
      ...
      showStatus(panel, "Conexión completada para " + (data.displayPhoneNumber || "el número BPGO") + ".", "success");
  ...
  function install() {
    document.querySelectorAll("[data-whatsapp-onboarding]").forEach(function (panel) {
    ...
    if (testPanel) card.insertBefore(panel, testPanel);

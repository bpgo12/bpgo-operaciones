...
window.bpInitMap=()=>{map=L.map("map",{center:[C.mapCenter.lat,C.mapCenter.lng],zoom:C.mapZoom,maxZoom:22,editable:true});const streets=L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxNativeZoom:19,maxZoom:22,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'}),satellite=L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxNativeZoom:18,maxZoom:22,attribution:"Tiles &copy; Esri"});streets.addTo(map);L.control.layers({"OpenStreetMap":streets,"Satelital":satellite},null,{position:"topright"}).addTo(map);map.on("click",e=>{hideMapContextMenu();mapClick(e.latlng)});map.on("contextmenu",e=>showMapContextMenu(e.latlng,e.originalEvent,null));const mapElement=$("map");mapElement.addEventListener("contextmenu",event=>{event.preventDefault();const latLng=map.mouseEventToLatLng(event);setTimeout(()=>{if(performance.now()-lastContextMenuAt>100)showMapContextMenu(latLng,event,null)},0)});if(!$("cableLegend")){const legend=document.createElement("div");legend.id="cableLegend";legend.className="map-legend";legend.innerHTML=`<div><i style="background:${CABLE_CLASS_COLORS.TRK}"></i>Troncal</div><div><i style="background:${CABLE_CLASS_COLORS.DST}"></i>Distribución</div><div><i style="background:${CABLE_CLASS_COLORS.DRP}"></i>Drop</div><div class="legend-divider">Cliente: <b class="level-dot good"></b>óptimo <b class="level-dot warning"></b>revisar <b class="level-dot critical"></b>crítico</div>`;document.querySelector("main").appendChild(legend)}render();setTool("select")};
...
db.features.forEach(f=>{if(f.type==="TRK"){if(!$("layerFiber").checked)return;let cableClass=f.cableClass||"TRK";let line=L.polyline(f.geometry.coordinates.map(([lng,lat])=>[lat,lng]),{color:CABLE_CLASS_COLORS[cableClass]||CABLE_CLASS_COLORS.TRK,weight:5}).addTo(map);line.on("click",()=>select(f.id));line.on("contextmenu",e=>showMapContextMenu(e.latlng,e.originalEvent,f.id));if(selectedId===f.id&&tool==="select"&&line.enableEdit){line.enableEdit();line.on("editable:vertex:dragend editable:vertex:new editable:vertex:deleted",()=>syncLine(f,line))}overlays.set(f.id,line)}
...
function setTool(t){tool=t;document.querySelectorAll("[data-tool]").forEach(b=>b.classList.toggle("active",b.dataset.tool===t));if(t!=="fiber"){draft=null;removeOverlay(overlays.get("_draft"))}$("newCableClass").closest("label").style.display=t==="fiber"?"block":"none";$("toolHelp").textContent=t==="select"?"＋ Clic derecho en el mapa para agregar elementos":t==="asset"?"Haz clic en el mapa.":"Elige origen, agrega vértices y elige destino.";$("mapStatus").textContent="Modo: "+t}
function showMapContextMenu(latLng,domEvent,cableId){
  ...
  menu.classList.remove("hidden");menu.style.left=`${Math.min(x,window.innerWidth-230)}px`;menu.style.top=`${Math.min(y,window.innerHeight-menu.offsetHeight-10)}px`
}
